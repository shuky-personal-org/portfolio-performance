...
public class IBFlexStatementExtractorTest
{
    ...
    public void testIBFlexStatementFile27() throws IOException
    {
        ...
        var transfers = results.stream().filter(TransactionItem.class::isInstance)
                        .map(i -> (PortfolioTransaction) i.getSubject()).toList();

        assertThat(transfers.size(), is(2));

        var eurTransfer = transfers.stream()
        ...
        assertThat(eurTransfer.getUnit(Unit.Type.GROSS_VALUE).isPresent(), is(false));

        var usdTransfer = transfers.stream()
